...
if query_type == 'User Types':
    ...
    with viz_col:
        fig = px.pie(user_types_df, values='count', names='usertype', title='User Types Visualization',
                     color='usertype', color_discrete_map={'customer': '#FFA500', 'member': '#0000FF'})
    ...
    with data_col:
        ...
        st.dataframe(bike_types_df, use_container_width=True, hide_index=True, column_order=['bike type', 'count'])
    with viz_col:
        fig = px.pie(bike_types_df, values='count', names='bike type', title='Bike Types Visualization',
                     color='bike type', color_discrete_map={'electric_bike': '#006400', 'docked_bike': '#00FFFF', 'classic_bike': '#0000FF'})
        fig.update_traces(textposition='inside', textinfo='percent+label')
        st.plotly_chart(fig, use_container_width=True, theme='streamlit')
    ...
    bike_types_list = list(bike_types)
    bike_types_df = pd.DataFrame(bike_types_list, columns=['count', 'member_type', 'bike_type'])
    with data_col:
        st.caption("Bike Types Used By Members Data")
        st.dataframe(bike_types_df, use_container_width=True, hide_index=True, column_order=['member_type', 'bike_type', 'count'])
    with viz_col:
        fig = px.bar(bike_types_df, x='bike_type', y='count', color='bike_type',
    ...
    user_average_duration_df = pd.DataFrame(user_average_duration_list, columns=['member_type', 'average_duration'])
    user_average_duration_df['average_duration'] = user_average_duration_df['average_duration'] / 60000
    with data_col:
        st.caption("Average trip duration per user types")
        st.dataframe(user_average_duration_df, use_container_width=True, hide_index=True, column_order=['member_type', 'average_duration'])
    with viz_col:
        fig = px.bar(user_average_duration_df, x='member_type', y='average_duration', color='member_type',
                     labels={'member_type': 'Member Type', 'average_duration': 'Average Duration (in minutes)'},
    ...
    popular_start_df = pd.DataFrame(popular_stations_data['popular_start_stations'], columns=['count', 'start station name'])
    popular_end_df = pd.DataFrame(popular_stations_data['popular_end_stations'], columns=['count', 'end station name'])

    with data_col:
        st.caption("Popular Start Stations")
        st.dataframe(popular_start_df, hide_index=True, use_container_width=True, column_order=['start station name', 'count'])

        st.caption("Popular End Stations")
        st.dataframe(popular_end_df, hide_index=True, use_container_width=True, column_order=['end station name', 'count'])

    with viz_col:
        fig_start = px.bar(popular_start_df, x='start station name', y='count',
        ...
        fig_end = px.bar(popular_end_df, x='end station name', y='count',
    ...
    with data_col:
        ...
        st.dataframe(peak_hours_df, hide_index=True, use_container_width=True, column_order=['hour', 'trip_count'])
